...
function makeSwitch(x){
  if (switchNumb){return articles["legumes"][x];}
  return articles["fruits"][x];
}
...
function initPage(){
  // trie les tableaux
  setSelect(Object.keys(articles["fruits"]).sort(cmpArt));  // affiche la liste
...
function choix(r){ // selon le choix via bouton-radio r, fabrique la liste demandée
  ...
  setSelect((r.value == 'F') ? (switchNumb = 0, Object.keys(articles["fruits"])) : ( switchNumb = 1, Object.keys(articles["legumes"])));
  console.log(switchNumb);
}
